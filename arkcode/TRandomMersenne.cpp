...
void TRandomMersenne::RandomInit(uint32 seed) {
	...
	if (convert.i[1] == 0x3FF00000) Architecture = RANDOM_LITTLEENDIAN;
	else if (convert.i[0] == 0x3FF00000) Architecture = RANDOM_BIGENDIAN;
	else Architecture = RANDOM_NONIEEE;
}
...
uint32 TRandomMersenne::BRandom() {
	...
	if (mti >= MERS_N) {
		...
		const uint32 UPPER_MASK = uint32(-1L << MERS_R);      // upper (32 - MERS_R) bits
...
double TRandomMersenne::Random() {
	...
	// RANDOM_LITTLEENDIAN architecture):
	switch (Architecture) {
	case RANDOM_LITTLEENDIAN:
		convert.i[0] = r << 20;
		convert.i[1] = (r >> 12) | 0x3FF00000;
		return convert.f - 1.0;
	case RANDOM_BIGENDIAN:
		convert.i[1] = r << 20;
		convert.i[0] = (r >> 12) | 0x3FF00000;
		return convert.f - 1.0;
	case RANDOM_NONIEEE: default:
		;
	}
...
int32 TRandomMersenne::IRandom(int32 min, int32 max) {
	// output random integer in the interval min <= x <= max
	int32 r;
	double f = Random();
	//r = int((max - min) * f + 0.5f) + min; // multiply interval with random and truncate
	double dMin = double(min);
	double dMax = double(max);
	double dSub = dMax - dMin + 1;
	double dRand = dSub * f;
	r = int32(dRand) + min;
